...
extension BulbFittingFamily {
    static let defaultSize = "GU10"

    static let catalog: [BulbFittingFamily] = [
        BulbFittingFamily(
            name: "Edison screw (E)",
    ...
    static func imageAsset(for size: String) -> String {
        catalog.first(where: { $0.sizes.contains(size) })?.imageAsset ?? "gu10"
    }
}
